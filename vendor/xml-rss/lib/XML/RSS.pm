...
sub _handle_start {
    ...
    elsif (($self->_current_element eq 'channel') && ($parser->namespace($el))) {
        # Backward compatible atom:link
        my $prefix = (grep { $el_ns eq $self->{namespaces}{$_} } keys %{$self->{namespaces}})[0];

        if ($prefix eq 'atom' && exists $attribs{rel} && $attribs{rel} eq 'self') {
            $self->{'channel'}{$prefix}{$el} = $attribs{href};
        }
    }
}
